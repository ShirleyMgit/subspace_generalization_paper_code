...
output_dir = 'subSpaceGener_9PCs';

slName = 'L100_projMat';
base = 'C:\Users\User\Documents\fMRI_EXP\';
spm_path =  "C:\Users\User\Documents\spm12";
output_path = fullfile(base, 'group_level_stats', output_dir, 'fsl_normalization');
data_path = fullfile(base, 'fsl_norm_results', output_dir);
%addpath(genpath(fullfile(data_path,rsa_tool_path)))
addpath(spm_path)
ana_name = 'standard_L100_FSLcleaned280920b';

addpath(fullfile('C:\Users\User\Documents\fMRI_EXP\Alon','from_git', 'util'));
addpath ('C:\Users\User\Documents\fMRI_EXP\Alon\from_git\util')
...
parfor iVox = 1:nVox
    ...
    if rem(iVox, 10000)==0
        disp(['iVox', num2str(iVox)])
    end
end
tStats_same = reshape(tStats_same_linVox,niiDims);
save4Dnii(tStats_same,V,fullfile(output_path,'tStat_ClSameMinusSameStim.nii'))
tStats = reshape(tStats_linVox,niiDims);
save4Dnii(tStats,V,fullfile(output_path,'tStat_ClStrMinusNothing.nii'))
