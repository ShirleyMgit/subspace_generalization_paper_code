...
r_random = 1;
index_random_v = index_random(1:num_grids/r_random);
index_phase = index_all;
index_phase(index_random_v) = [];

r_voxel = length(index_random_v) / n_voxels;
...
for n1 = 1:sqrt(n_voxels)
    for n2 = 1:sqrt(n_voxels)
        ...
        grid2average = mean([env1_data(ind2average>0,  :); env1_data(index_random_v(1+(nv-1)*r_voxel:r_voxel*nv), :)]);
        env1_voxels_m(n1 + (n2-1)*sqrt(n_voxels), :) = grid2average;
        grid2average = mean([env2_data(ind2average>0,  :); env2_data(index_random_v(1+(nv-1)*r_voxel:r_voxel*nv), :)]);
