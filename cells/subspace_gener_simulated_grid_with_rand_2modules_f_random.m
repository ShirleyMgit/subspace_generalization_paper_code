...
v_r_random = [0, 1];%0:0.02:1;
index_all = 1:num_grids;

p_val_effect = zeros(2, length(v_r_random),1);
mean_auc = zeros(2, length(v_r_random),1);
...
for isnoise = 1:2
    ...
    for n=1:length(v_r_random)
        ...
        mean_auc(isnoise,n) = 0.5 * ((sum(cumsum_var_grid_same) + sum(cumsum_var_grid_dif))/n_voxels);
        ...
        grid_effect(isnoise, n) = grid_effect1;
        ...
        p_val_effect(isnoise, n) = p_val_grid;
...
figure(11)
subplot(1,3,1)
plot(v_r_random, auc_within(1, :), 'k', 'LineWidth', 2)
hold on
plot(v_r_random, auc_between(1, :), 'g', 'LineWidth', 2)
hold on
plot(v_r_random, auc_within(2, :), '-.k', 'LineWidth', 2)
hold on
plot(v_r_random, auc_between(2, :), '-.g', 'LineWidth', 2)
ylabel('AUC')
xlabel('franction random')
ylim([0.5, 1])
subplot(1,3,2)
plot(v_r_random, grid_effect(1, :), 'k', 'LineWidth', 2)
hold on
plot(v_r_random, grid_effect(2, :), '-.k', 'LineWidth', 2)
ylabel('AUC_{within - between}')
xlabel('franction random')
ylim([-0.25, 0.25])
subplot(1,3,3)
plot(v_r_random,p_val_effect(1, :), 'b', 'LineWidth', 2)
hold on
plot(v_r_random,p_val_effect(2, :), '-.k', 'LineWidth', 2)
